...
class UserAuthorityController extends \webadmin\BController
{
	// 执行前
    public function beforeAction($action){
        Yii::$app->controller->pageTitle = Yii::t('datacenter', '用户数据权限');
    ...
    public function actionTree()
    {
        ...
        return $this->render('tree', [
            'treeData' => DcUserAuthority::treeData(),
        ]);
    }
    */
    ...
    {
        $model = $this->findModel($id);
        $aModel = new \datacenter\models\DcUserAuthority();
        
        if ($aModel->load(Yii::$app->request->post()) && $aModel->saveAuthority($model['id'])) {
            Yii::$app->session->setFlash('success',Yii::t('common', '对象信息修改成功'));
            return $this->redirect(!empty(Yii::$app->session[$this->id]) ? Yii::$app->session[$this->id] : ['index']);
        }elseif($aModel->hasErrors()){
            Yii::$app->session->setFlash('error',implode('<br>', $aModel->getErrorSummary(true)));
        }
    ...
    /*public function actionDelete()
    {
        ...
        if($id && ($models = DcUserAuthority::findAll($id))){
        	$transaction = DcUserAuthority::getDb()->beginTransaction(); // 使用事务关联
